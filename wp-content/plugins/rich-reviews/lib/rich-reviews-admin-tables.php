...
class Rich_Reviews_Table extends WP_List_Table {
		...
		if (isset($_REQUEST['review'])) {
			...
			if (!empty($ids)) {
				foreach ($ids as $id) {
					...
					switch ($this_action) {
						case 'approve':
							$wpdb->update($rr_sqltable, array('review_status' => '1'), array('id' => $id));
							break;
						case 'pending':
							$wpdb->update($rr_sqltable, array('review_status' => '0'), array('id' => $id));
							break;
						case 'delete':
	...
	function prepare_items($flag = 'pending') {
		...
		$get_order_by = sanitize_text_field($orderBY);
		$orderby = (!empty($get_order_by)) ? $get_order_by : 'id';

		$ORDER = isset($_GET['order']) ? $_GET['order'] : '';
		$get_order = sanitize_text_field($ORDER);
