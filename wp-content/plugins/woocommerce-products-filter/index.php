    ...
    public function init() {
        ...
        if ($this->get_option('per_page') > 0 AND $this->is_isset_in_request_data($this->get_swoof_search_slug())) {
            if (version_compare(PHP_VERSION, '5.3.0', '<=')) {
                add_filter('loop_shop_per_page', create_function('$cols', "return {$this->get_option('per_page')};"), 20);
            } else {
...
//                        );
    ...
    public function woof_products($atts, $is_prediction = false) {
        ...
        <?php if ($is_ajax == 1): ?>
            ...
            <div id="woof_results_by_ajax" data-count="<?php echo $products->found_posts ?>"  class="woof_results_by_ajax_shortcode container" data-shortcode="<?php echo $shortcode_txt ?>">
                <?php
                //endif;
                //ajax compatibility
                    ...
                    if (count($need_array)) {
                        ...
                        if (array_shift($rate)) {
                            if (isset($variations[$attr_key]["image_id"]) AND $variations[$attr_key]["image_id"]) {
                                $image_size = apply_filters('large', 'woocommerce_thumbnail');
