...
{	
	...
	class ced_wramvp_Frontend
	{
		...
		public function ced_rvmv_show_recent_viewed_products($atts)
		{
			...
			<?php if(!is_product() || !is_cart() || !is_shop() || !is_checkout()){?>
				<div class="container review_recently">	
			<?php }?>	
					<div class="ced row justify-content-center" data-rows="<?php echo  $number_of_products_in_row ?>">
						<p class="last-word" ><?php 
						  $wrvp_viewed_title = get_option( CED_RVMV_PREFIX.'_wrvp_viewed_title', null);
						  if($wrvp_viewed_title){ echo $wrvp_viewed_title;} else { echo __('Recently viewed products','recently-viewed-and-most-viewed-products');} ?></p>
