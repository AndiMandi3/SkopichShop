...
class ACF_Frontend_Form_Widget extends Widget_Base
{
        ...
        if ( 'logged_in' == $settings['who_can_see'] ) {
            ...
            if ( !is_user_logged_in() ) {
                ...
                if ( isset( $settings['dynamic'] ) ) {
                    ...
                    if ( $settings['post_to_edit'] == 'current_post' ) {
                        $post_id = $post->ID;
    ...
    protected function render()
    {
        ...
        $defaults = $new_post = $show_title = $show_content = $show_form = $display = $message = $fields = $field_groups = $fields_exclude = false;
        ...
        $submit_button .= '<input' . $disabled_submit . ' type="submit" class="acfef-submit-button acf-button button button-primary' . $hidden_submit . '" value="%s" /></div>';
        $module = ACFEF_Module::instance();
        $hidden_fields = [
        ...
        if ( isset( $settings['saved_drafts'] ) ) {
            $form_args['saved_drafts'] = [
                'saved_drafts_label' => $settings['saved_drafts_label'],
                'saved_drafts_new'   => $settings['saved_drafts_new'],
                'show_in_modal'      => $settings['show_in_modal'],
                'open_in_modal'      => $settings['open_modal'],
            ];
        }
        
        if ( isset( $settings['save_progress_button'] ) && $settings['save_progress_button'] && $settings['new_post_status'] != 'draft' ) {
            $form_args['save_progress'] = '<br><div class="acfef-draft-buttons">';
            if ( $settings['saved_draft_desc'] ) {
                $form_args['save_progress'] .= '<p class="description"><span class="btn-dsc">' . $settings['saved_draft_desc'] . '</span></p>';
            }
            $form_args['save_progress'] .= '<input formnovalidate type="submit" class="acfef-draft-button acf-button button button-secondary" value="' . $settings['saved_draft_text'] . '" name="acfef_save_draft" /></div>';
        }
